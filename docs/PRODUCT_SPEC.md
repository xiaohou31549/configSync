...
- 支持在发布包中内置 GitHub App 配置，使终端用户默认只需完成授权与仓库选择
- 拉取用户可访问仓库
- 本地维护 Secret
- 按仓库搜索、筛选、多选
- 批量同步并展示结果
- GitHub App 本地配置保存；若用户手工覆盖内置配置，则 `Client Secret` 与其余元数据一起保存在本地配置文件
- GitHub 登录会话与本地 Secret 持久化不依赖 macOS Keychain，避免首次启动触发系统钥匙串读取弹窗
